...
class ProToolOrchestrator:
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        # 1. تشغيل مخرجات الموديل للرصد والتحليل
        detections = self.detector.detect_elements(frame, self.current_roi)
        
        # 2. في وضع الـ CI بنثبت الإحداثيات الحقيقية المأخوذة من سكرين شوت شاشتك بالظبط
        if self.is_ci:
            # الإحداثيات مأخوذة بالبيكسل الصافي لسنتر الكور والمسارات الأصلية في صورتك
            cue_pos = (573, 564)              # مركز الكرة البيضاء بالظبط
            self.locked_ball_pos = (638, 715)  # مركز الكرة البني المستهدفة رقم 7
            pock_pos = (495, 246)              # الجيب العلوي الأوسط المتجه إليه الخط
        else:
        ...
        # رسم المربعات والدوائر الاسترشادية العامة للـ YOLO
    ...
    def run_static_test(self, input_path: str, output_path: str):
        ...
        print(f"Static test build finished. Saved to {output_path}")
